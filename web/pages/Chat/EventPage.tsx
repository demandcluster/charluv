  ...
  return (
    <div class="evt-root" role="region" aria-label="Start an event">
      <div class="evt-page">
        ...
        <div
          class="evt-help"
          style="border: 1px solid var(--bg-700); border-radius: 8px; padding: 8px 12px; margin-bottom: 12px;"
        >
          Heads up: events run on a different AI model than normal chat — it's built to direct
          multi-character scenes, so replies may read a little differently.
        </div>
        ...
        <div class="evt-block">
          ...
          <Show
            when={chars().length}
            fallback={<div class="evt-empty">You have no characters yet. Create one first.</div>}
          >
                  ...
                  {(c) => {
                    ...
                    return (
                      ...
                      >
                        <Show
                          when={c.avatar}
                          fallback={<div class="evt-ph">{c.name?.[0]?.toUpperCase() || '?'}</div>}
                        >
                          <img
