...
export async function generateImage({ chatId, messageId, onDone, ...opts }: GenerateOpts) {
  ...
  // Characters with a trained LoRA get their identity from the LoRA itself, so
  // the saved appearance prompt is skipped: it bakes in the creation outfit and
  // setting, which fights whatever the scene caption says they're wearing/doing.
  // Without a LoRA the appearance prompt is the only visual anchor, so it still
  // leads (first so the token trim below keeps it).
  const hasLora = !!entities.char?.loraName
  const appearance = hasLora ? undefined : entities.char?.appearance?.trim()
