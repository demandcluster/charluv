...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName('register')
    .setDescription(
      'Send your Charluv username and ID to the bot to be granted the appropriate roles'
    )
    ...
    .addStringOption((option) =>
      option.setName('id').setDescription('Your ID from your profile').setRequired(true)
    )
  ...
  async execute(interaction: any) {
    ...
    if (user?.username === username) {
      const registeredRole: Role | undefined = guild.roles.cache.find(
        (role: Role) => role.name === 'registered'
      )
      const premiumRole: Role | undefined = guild.roles.cache.find(
        (role: Role) => role.name === 'premium'
      )
      ...
      if (user?.premium === true) {
        ...
        await interaction.reply({
          content: 'Thank you for being a premium member!',
          ephemeral: true,
        })
      }
