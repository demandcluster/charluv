...
type NewScenario = Pick<
  AppSchema.Scenario,
  '_id' | 'kind' | 'name' | 'charId' | 'xp' | 'greeting' | 'prompt'
>

export async function ensureInitialScenario() {
  const scenario = await db('scenario').findOne({ kind: 'scenario' })
  if (scenario) return
  const firstCharacter = await db('character').findOne({
    kind: 'character',
    name: { $eq: 'Julia' },
  })
  ...
  await createScenario({
    ...
    name: 'Just matched',
    prompt:
      '{{char}} and {{user}} just matched, {{char}} will take things slow, {{user}} should be patient.',
...
export async function getScenario(charId: string, char: AppSchema.Character) {
  ...
  let sCharId = charId
  if (char?.parent) {
    let originalCharacter = await db('character').findOne({ _id: char.parent })
