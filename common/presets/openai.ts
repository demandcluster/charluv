...
export const openaiPresets = {
  openai: {
    ...
    topP: 1,
    antiBond: false,
    // Use the Charluv system-prompt template (same as horde/kobold). The
    // character is inserted via the template's {{personality}}/{{scenario}}
    // holders, and the built-in <roleplay> system prompt applies when no
    // per-preset system prompt is set.
    systemPrompt: '',
